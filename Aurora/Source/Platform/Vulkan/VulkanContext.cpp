...
namespace Aurora {
	VulkanContext::VulkanContext(GLFWwindow* windowHandle) : m_WindowHandle(windowHandle), m_SwapChain(windowHandle){
		if (!windowHandle) {
			AU_CORE_LOG_ERROR("Window handle is NULL!");
		}
	...
	void VulkanContext::SwapBuffers() {
		...
		if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
			...
			AU_CORE_LOG_CRITICAL("Failed to present swap chain image!");
		}
	...
	void VulkanContext::createVulkanInstance() {
		if(m_AreValidationLayersEnabled && !m_ValidationLayer.checkValidationLayerSupport()) {
			AU_CORE_LOG_CRITICAL("VALIDATION LAYERS ARE REQUESTED, BUT NOT AVAILABLE!\n");
		}
		...
		if (vkCreateInstance(&createInfo, nullptr, &m_Instance) != VK_SUCCESS) {
			AU_CORE_LOG_CRITICAL("FAILED TO CREATE VULKAN INSTANCE!");
		}
	}
	...
	void VulkanContext::createSurface() {
		if (glfwCreateWindowSurface(m_Instance,
									m_WindowHandle,
									nullptr, &m_SwapChain.GetSurface()) != VK_SUCCESS) {
			AU_CORE_LOG_CRITICAL("FAILED TO CREATE WINDOW SURFACE!");
		}
	}
	...
	void VulkanContext::pickPhysicalDevice() {
		...
		if (deviceCount == 0) {
			AU_CORE_LOG_CRITICAL("Failed to find GPUs with Vulkan support!");
			return;
		}
		...
		if (m_PhysicalDevice == VK_NULL_HANDLE) {
			AU_CORE_LOG_CRITICAL("Failed to find a suitable GPU!");
		}
	}
	...
	QueueFamilyIndices VulkanContext::findQueueFamilies(const VkPhysicalDevice& device) {
		...
		for (const auto& queueFamily : queueFamilies) {
			...
			vkGetPhysicalDeviceSurfaceSupportKHR(device, i, m_SwapChain.GetSurface(), &presentSupport);
	...
	void VulkanContext::createRenderPass() {
		...
		if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
			AU_CORE_LOG_CRITICAL("Failed to create render pass\n");
		}
	}
	...
	VkShaderModule VulkanContext::createShaderModule(const std::vector<char>& code) {
		...
		if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
			AU_CORE_LOG_CRITICAL("Failed to create shader module\n");
		}
	...
	void VulkanContext::createFramebuffers() {
		...
		for (size_t i = 0; i < swapChainImageViews.size(); i++) {
			...
			if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) {
				AU_CORE_LOG_CRITICAL("Failed to create framebuffer\n");
			}
		}
	}
	...
	void VulkanContext::createCommandPool() {
		...
		if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
			AU_CORE_LOG_CRITICAL("Failed to create command pool\n");
		}
	}
